...
import { Apple, ApplePie, AppleSlice, Compote, PiePastry } from './models';
...
class BakingServiceImpl {
  ...
  public bakeApplePie(piePastry: PiePastry, compote: Compote, appleSlices: AppleSlice[]): Observable<ApplePie> {
    if (appleSlices.length !== 64) {
      return throwError(() => new Error('Cannot make apple pie without 64 apple sclices'));
    }

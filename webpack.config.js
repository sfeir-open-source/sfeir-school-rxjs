...
module.exports = function(env = {}, args) {
  ...
  const stepArg = process.argv.find(arg =>
    arg.startsWith('--step-')
  );
  let stepDir = stepArg && stepArg.replace('--step-', '');
  const STEPS_DIRECTORY = 'steps/';
  stepDir =
    stepDir === '' || stepDir === 'true'
      ? DEFAULT_STEP
      : `${STEPS_DIRECTORY}${stepDir}`;
  ...
  console.log(
    `=== Running ${
      env.step ? `step ${env.step}` : `the base app`
    } ===`
  );
  ...
  try {
    ...
    try {
      ...
      paths.indexTemplate = path.join(
        'common/default-for-steps/',
        stepGenericName,
        landingFile
      );
      fs.accessSync(paths.indexTemplate, fs.constants.R_OK);
